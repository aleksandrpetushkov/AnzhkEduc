...
namespace WhLib
{
	...
	public class WH
	{
		...
		//Полностью разобрали
		public void InsertGoods(string st = null, int i = 0, int k=0, int y=10) //Если ничего не передали (вызвали метод без параметра) то переменная st будет указывать на null (или другое значение например st="Hello world", проинициализированные переменные указываются справа от не проинициализированных переменных)) ссылочные типы инициализируются по дефолту null, значимые типы инициализируются по умолчанию 0 (int i = 0)
		{
